...
const TCP_READ_BUFFER_SIZE: usize = 4096; // Bytes to read from TCP socket at a time
// How many i16 samples to bundle per message on the mpsc channel.
// E.g., 1600 samples at 16kHz = 100ms of audio. (1600 * 2 bytes = 3200 bytes)
const SAMPLES_PER_CHUNK_TO_SEND: usize = 1600;

pub struct TcpAudioInput {
    address: String,
    sample_rate: u32,
    channels: u16, // Assumed to be 1 (mono) by the processing logic for Gemini
...
impl TcpAudioInput {
    pub fn new(address: String, sample_rate: u32, channels: u16) -> Self {
        ...
        Self {
            address,
            sample_rate,
            channels, // Store it, but processing logic below focuses on mono
...
impl AsyncAudioInput for TcpAudioInput {
    ...
    async fn start_stream(&mut self) -> Result<Receiver<Vec<i16>>> {
        ...
        let (tx_audio_chunks, rx_audio_chunks) = mpsc::channel::<Vec<i16>>(100); // Buffer up to 100 audio chunks
    ...
    async fn stop_stream(&mut self) -> Result<()> {
        self.stop_signal.notify_waiters(); // Notify the processing loop to stop
...
async fn process_tcp_to_audio_chunks(
    ...
    let mut byte_buffer = Vec::with_capacity(TCP_READ_BUFFER_SIZE * 2); // Temporary buffer for bytes
    ...
    loop {
        tokio::select! {
            biased; // Give stop_signal higher priority
            ...
            read_result = stream.read(&mut read_buf) => {
                match read_result {
                    ...
                    Ok(bytes_read) => {
                        ...
                        // Process bytes into i16 samples
                        while byte_buffer.len() >= 2 { // Enough for one i16 sample
                            let sample_bytes = [byte_buffer[0], byte_buffer[1]];
                            let sample = i16::from_le_bytes(sample_bytes);
                            pcm_samples_collected.push(sample);
                            byte_buffer.drain(0..2); // Remove processed bytes
                    ...
                    Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                        // This should ideally not happen frequently with async read, but good to handle
    ...
    // Send any remaining collected samples before exiting
